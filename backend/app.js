...
const sequelize = new Sequelize({dialect: 'sqlite', storage: 'parcial.db'});
...
app.get('/musculos', async (req, res) => {
  try {
      ...
      const data = musculos.map(musculo => musculo.dataValues)
      return res.json(data)
  } catch (error) {
      return res.status(404).json({ error: "No encontre el mùsculo :(" })
